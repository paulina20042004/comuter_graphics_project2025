...
namespace Core {
    void RenderContext::initFromAssimpMesh(aiMesh* mesh) {
        minVertexPos = glm::vec3(FLT_MAX);  // Najwiêksza mo¿liwa wartoœæ typu float
        maxVertexPos = glm::vec3(-FLT_MAX); // Najmniejsza mo¿liwa wartoœæ typu float

        std::vector<float> textureCoord;
        std::vector<unsigned int> indices;

        // Szukanie wartoœci min i max dla pozycji wierzcho³ków
        for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
            aiVector3D vertex = mesh->mVertices[i];

            minVertexPos.x = std::min(minVertexPos.x, vertex.x);
            minVertexPos.y = std::min(minVertexPos.y, vertex.y);
            minVertexPos.z = std::min(minVertexPos.z, vertex.z);

            maxVertexPos.x = std::max(maxVertexPos.x, vertex.x);
            maxVertexPos.y = std::max(maxVertexPos.y, vertex.y);
            maxVertexPos.z = std::max(maxVertexPos.z, vertex.z);
